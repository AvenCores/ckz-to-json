...
- выбор `.ckz`-файла: диалог выбора файла ОС (Windows / macOS / Linux + zenity/kdialog),
  нумерованный список в терминале, drag&drop пути или флаг `-i`;
- ввод пароля: скрытый ввод в терминале (без эха), флаг `-p` или переменная `CKZ_PASSWORD`;
...
Требуется Go 1.22+ (внешних зависимостей нет).
