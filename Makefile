...
.PHONY: all test vet release build run winres clean
...
# Regenerate Windows version resources / icon embedding (needs network once)
winres:
	go run github.com/tc-hib/go-winres@latest make --arch amd64,arm64,386 \
	  --in cmd/$(APP)/winres.json --out cmd/$(APP)/winres
